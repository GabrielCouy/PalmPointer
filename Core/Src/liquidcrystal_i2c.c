...
void updateScreen(bool on, uint8_t DPI, uint8_t mode) {
  char topLine[16] = "MODE   ";

  if (on) {
	  strcat(topLine, " ON   DPI");

  }
  else {
	  strcat(topLine, "OFF   DPI");
  }

  HD44780_SetCursor(0,0);
  HD44780_PrintStr(topLine);

  if (mode == 1) {
	  char base[16] = "Vertical     ";
